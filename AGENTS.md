...
- The profile quick actions: **Sertifikalarım** goes to `/profile/certificates` (core now has `GET /v1/certificates/me`, #46), **Öğrenci Kartını Eşle** checks NFC availability and opens `NfcScanOverlay` (`NfcService`, ISO 14443-A only) — but the read UID is not sent anywhere yet (core has `POST /v1/skypass/card-bind`, #43). **QR'ı Göster** flips the SkyPass card through `SkyPassCardController` (same as tapping the card).
