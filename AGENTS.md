...
- **Top level** (full screen, root navigator): `/`, `/auth`, `/notification`, `/webview`, `/settings` (sub-route: `/settings/contact`)
...
| `dio`                      | All REST calls                                                                                             |
| `image_picker`             | Event cover and news image (`MediaService.pickImage`, `CoverPicker`; upload errors 413/415/429/503 become Turkish text through `ApiException.userText`) — **native dependency**, iOS needs `NSPhotoLibraryUsageDescription` |
| `flutter_localizations`    | App locale is Turkish (`main_app.dart`); date/time pickers and system menus are Turkish |
| `webview_flutter`          | Hesap Merkezi (`AccountCenterPage`) in-app, and the `/webview` fallback — **native dependency**; its cookie jar is separate from the login browser, which is why the sites need the web handoff |
...
- The news on the home page come from SkyCMS `News` (`NewsService`/`NewsProvider`, public read, not enveloped; slug is generated from the title and will be used for deep links #44). Users allowed by `User.canManageNews` (`cms:access` on the `skyapp` client **and** a YK/DK/ADMIN group — the CMS rule) get a FAB on the home page and an edit button on the news detail; `NewsEditPage` posts/puts to CMS. No delete (CMS has no endpoint). The news image is picked from the gallery (`CoverPicker`, square — the event cover picker is square too) and uploaded to core `POST /v1/media` (`MediaService.uploadImage`) on save **without a `purpose`**, so it is stored as `legacy` — the `cms_image` purpose exists in core's catalogue but answers `422 purpose_not_available` until CMS gets a service client. Risk reported to the backend: once `legacy` falls to the strict rule, an image nothing attaches expires in 24 h and news pictures would disappear; the returned CDN URL goes into the CMS `heroImage` field (a URL). "Görseli Kaldır" clears it. Event covers use the same `MediaService` but store the media id. The notifications (`NotificationService`) list is empty until the backend adds notifications (issue #45).
...
- **Hesap Merkezi (`my.yildizskylab.com`) replaces the app's own account page.** Ayarlar → hesap satırı opens `HandoffSheet` (`core/widgets/`) — a full-height modal sheet (94 % of the screen, the app's usual sheet corners) holding an **in-app first-party WebView** (`webview_flutter`); its header is an X on the left and a centred "Hesap Merkezi" title, so no route is involved; Custom Tabs / SFSafariViewController are forbidden for this flow, so it is the one place we do not use `WebviewService`. **Web handoff** (`HandoffService`, contract `mobile-web-handoff-contract.md`, 2026-09-23): `POST https://e.yildizskylab.com/realms/e-skylab/sky-handoff/v1/handoffs` with the app's Bearer token and `{"target": "<client>", "path": "/"}` returns `{handoffUrl, proof, expiresIn ≤ 45}`; the WebView loads that URL **with the `X-Sky-Handoff-Proof` header on the first request only** and the user lands signed in. Targets are server-side (`account-center` today, `skyforms` for SKYFORMS — `HandoffTarget`), so new sites need no app release. The code is single-use: a new one is fetched on every open, never cached or logged. The old `my.yildizskylab.com/v1/native-handoff` endpoint is **no longer called** (removed 2026-09-24, the Keycloak endpoint went live). Failures land on `…/sky-handoff/v1/failed?reason=…`; `_onUrlChange` catches it (and the account center's own `/login?error=…`) and shows our own screen with a Turkish reason and a retry. Allowed hosts: the target's own origin plus `e.yildizskylab.com` and `login.microsoftonline.com` (the Microsoft hop is sudo-mode re-auth, do not block it); anything else opens in the system browser. The system back button goes through the WebView history first (`PopScope`), the X always closes; closing reloads the profile. The WebView takes touch gestures itself (`EagerGestureRecognizer`) so the page scrolls; the sheet is dragged shut from its header. **The same sheet opens SKYFORMS:** the event detail shows a **Başvuru Formu** row when `event.formUrl` is set — `HandoffService.pathOf(HandoffTarget.forms, url)` returns the path for `forms.yildizskylab.com` links (opened signed-in in the sheet) and `null` for anything else (Google Forms etc., opened in the browser sheet). **On logout `HandoffService.clearWebSession()` wipes the WebView cookies, local storage and cache** so the next user does not inherit the session. The account center hides its own top bar when opened this way — our AppBar is the only header. What lives there: kimlik, parola/TOTP/passkey, oturumlar, kulüp profili, yetkiler, hesap silme (still `ACCOUNT_ERASURE_MODE=off`). **Known gaps:** passkeys do not work in an Android WebView and need `assetlinks.json`/AASA on `yildizskylab.com`; a stale app login could hit `upstream_session_expired` (fixed server-side, re-login works around it).
...
- **Session QR attendance** (`SessionCheckInPage`, `features/calendar`, opened by **QR Okut** on the profile): the attendee scans the session QR shown on the projector. Core's QR (`GET /v1/sessions/{id}/qr`) only encodes `https://api.yildizskylab.com/v1/sessions/{id}`; `AttendanceService.sessionIdFromQr` accepts exactly that shape (https, API host, uuid). The page first reads the session (`GET /v1/sessions/{id}`: 404 → "Oturum bulunamadı", cancelled → not sent), then `POST /v1/sessions/{id}/check-in/me` (201 success, 409 already, 404 no ticket for this event). Success shows the session title and event name (day → event, best effort), stays on screen and refreshes `ActivityProvider`; errors fade after 5 s. The result card and message box are shared with the door scanner (`ScanResultCard`, `ScanMessage`). Known backend gap (reported, deliberately left): the QR is static and session ids are public, so check-in does not prove presence and there is no time-window/cancel check in core.
