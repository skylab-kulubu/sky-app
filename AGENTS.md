...
- **Certificates** (`/profile/certificates`, #46): `CertificateService` reads `GET /v1/certificates/me` (core issues them from session attendance per the event's `attendanceRule`); revoked ones are hidden, newest first, the issuer shows YK/DK as "SKY LAB" (`EventModel.ownerLabelFor`). Tapping a row opens the public PDF (`verifyUrl` + `/pdf`) in the browser sheet (`WebviewService`). Loading / error + retry / empty states and pull-to-refresh.
- The profile quick actions: **Sertifikalarım** goes to `/profile/certificates`, **Öğrenci Kartını Eşle** checks NFC availability and opens `NfcScanOverlay` (`NfcService`, ISO 14443-A only); the read UID (`NfcCard.normalizedHex`) is bound to the account with `SkyPassService.bindCard` → `POST /v1/skypass/card-bind` (409 = card bound to another account). Core no longer returns `studentCardUid` from `/v1/users/me`, so the app cannot show whether a card is bound. **QR'ı Göster** flips the SkyPass card through `SkyPassCardController` (same as tapping the card).
...
- **Door scanner** (`DoorScannerPage`, `features/calendar`): staff pick an event and a session, then scan attendees' SkyPass QR with `mobile_scanner`; each scan calls `POST /v1/sessions/{id}/check-in/skypass` (`DoorService`). The name shown after a scan is read from the token payload (`DoorService.holderFrom`, `iss == skypass`) — core's `/v1/skypass/verify` is privileged-only, so it is not used. Results: success / 409 already checked in / 404 no ticket for this event / 401 expired code / 403 no permission; the scanner stays open and ignores the same code for 4 s. A **QR Okut / Öğrenci Kartı** switch above the scanner area changes the mode: in card mode the camera is stopped and a "Kartı Okut" button reads the student card with `NfcService` and checks in with `{uid}` on the same endpoint (404 there means either the card is not bound or the person has no ticket — core does not distinguish). The card holder's name comes from `GET /v1/skypass/card?uid=`, which core allows only for YK/DK/ADMIN; for other staff the result is shown without a name. Sessions come from `/v1/events/{id}/days` + `/v1/event-days/{id}/sessions` (cancelled ones hidden; default = the one running now). Who may scan mirrors core `allowDoorCheckIn` in `User.canCheckIn`: YK/DK/ADMIN, leader/coordinator of the event's `ownerTeam`, or the user's id in `EventModel.doorStaffIds`. Members of a team with the group attribute `team_door_scan=true` may also scan in core, but that is not in the token, so the app cannot show them the button. On the profile the third quick action becomes **QR'ı Okut** (opens the scanner) when the user can scan at least one upcoming event; otherwise it stays **QR'ı Göster**.
